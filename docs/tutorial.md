...
```javascript
const dust = new Dust();
const compiled = dust.templateManager.compile('Hello {name}!');
```
...
```javascript
const substrHelper = function (chunk, ctx, bodies, params) {
	...
	let str = ctx.tap(params.str, chunk);
	let begin = ctx.tap(params.begin, chunk);
	let end = ctx.tap(params.end, chunk);
	let len = ctx.tap(params.len, chunk);
