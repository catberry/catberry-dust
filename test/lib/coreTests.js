 ...
 */
	{
		...
		tests: [
			...
			},
			{
				...
				error: "Template name should be a string",
 ...
 */
	{
		...
		tests: [
			...
			},
			{
				...
				context: { "emptyParamHelper": function (chunk, context, bodies,
					params) {
 ...
 */
	{
		...
		tests: [
			...
			},
			{
				...
				context: { list4: [
					{name: "Dog", number: [1, 2, 3]},
					{name: "Cat", number: [4, 5, 6]}
				] },
		...
		tests: [
			{
				...
				source: "{#people}{.name} is {?.age}{.age} years old.{:else}not" +
					" telling us their age.{/age}{/people}",
				context: {
					"name": "List of people",
					"age": "8 hours",
					"people": [
			...
			},
			{
				...
				options: {pathScope: "global"},
				context: { "nulls": [1, null, null, 2], "names": [
					{"name": "Moe"},
					{"name": "Curly"}
				] },
 ...
 */
	{
		...
		tests: [
			...
			},
			{
				...
				source: "{obj|j|s}",
				context: { obj: "<script>\\testBS\\ \rtestCR\r " +
					"\u2028testLS\u2028 \u2029testPS\u2029 \ntestNL\n " +
					"\ftestLF\f 'testSQ' \ttestTB\t /testFS/</script>" },
				expected: "<script>\\\\testBS\\\\ \\rtestCR\\r " +
					"\\u2028testLS\\u2028 \\u2029testPS\\u2029 " +
					"\\ntestNL\\n \\ftestLF\\f \\'testSQ\\' \\ttestTB\\t " +
					"\\/testFS\\/<\\/script>",
		...
		tests: [
			...
			},
			{
				...
				context: { helper: function (chunk, context, bodies,
					params) { return chunk.write(params.foo); } },
				expected: "10",
			...
			},
			{
				...
				context: { helper: function (chunk, context, bodies,
					params) { return chunk.write(params['data-foo']); } },
				expected: "dashes",
 ...
 */
	{
		...
		tests: [
			...
			},
			{
				...
				context: { "val1": "title", "val2": "A", "obj": { "name": [
					"A", "B"
				] } },
				expected: "AAA",
 ...
 */
	{
		...
		tests: [
			...
			},
			{
				...
				source: "{#hello/}",
				context: {"hello": function (chunk, context, bodies, params) {
					return chunk.map(function (chunk) {
						var a;
						a.slice(1);
						chunk.end();
 ...
 */
	{
		...
		tests: [
			...
			},
			{
				...
				expected: [
					"buffer text!spaces and new lines are nullified (by default). Booo",
